...
//    ui.treeWidget->setWindowIcon(QIcon(":/images/display.png")); 使用label替换
    ...
    //连接电池电压
    connect(&qnode,SIGNAL(battery_vel(_Float32,char)),this,SLOT(slot_update_battery(_Float32,char)));
    //连接图像话题
...
void MainWindow::slot_update_battery(_Float32 voltage, char percentage)
{
    //value*0.001,最大16811mV
    int vol = voltage * 0.001;
    ui.label_power_val->setText(QString::number(vol).mid(0,5)+"V");//只取前5个字符
