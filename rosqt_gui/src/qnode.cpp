...
#include <sstream>
#include "../include/rosqt_gui/qnode.hpp"
...
void QNode::image_callback(const sensor_msgs::ImageConstPtr &msg)
{
    ...
    cv_ptr = cv_bridge::toCvCopy(msg,msg->encoding);//msg->encoding编码格式
...
void QNode::battery_callback(const yzz_msgs::Battery &msg)
{
    emit battery_vel(msg.voltage,msg.percentage);
}
//按钮控制
...
void QNode::set_PTZ_vel(char k, float pan_speed, float tilt_speed)
{
    std::map<char, std::vector<float>>PTZBindings={
        {'I',{0,1}},
        {'<',{0,-1}},
        {'J',{1,0}},
        {'L',{-1,0}}};
    ...
    {
        qDebug()<< QString("currently angular_z:%1\t angular_y:%2").arg(NewPTZ.angular_z).arg(NewPTZ.angular_y);
        New_PTZ_pub.publish(NewPTZ);
    }
    ros::spinOnce();
}
