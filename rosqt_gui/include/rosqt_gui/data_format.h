...
enum Message_Id_Enum
{
    ...
    MSG_ID_GET_SENSOR = 5,       //获取传感数据
    MSG_ID_SET_HOLDER = 6,      //设置云台旋转、俯仰
    MSG_ID_GET_HOLDER = 7       //获取云台旋转、俯仰
};
...
//云台结构体
struct Data_Format_Holder
{
    float angular_z; //旋转0-180
    float angular_y; //俯仰0-180

    Data_Format_Holder(const double& wz=.0, const double& wy=.0)
        : angular_z(wz), angular_y(wy){}

    void set(const double& wz=.0, const double& wy=.0)
    {
        angular_z = wz; angular_y = wy;
    }
    void EndianSwapSet(const void *src)
    {
        FormatTools::EndianSwap(this, src, sizeof(angular_z), sizeof(Data_Format_Holder));
    }
};
...
struct Data_Format_SENSOR
{
    ...
    void EndianSwapSet(const void *src)
    {
        FormatTools::EndianSwap(this, src, sizeof(temperature), sizeof(Data_Format_SENSOR));
    }
};
